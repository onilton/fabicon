...
def getFeeds(url, enableMetaTagSearch = True, seenUrls = [], deepLevel = 0, debug = False, downloadDebug=False) :
	...
	if debug: print "FinalUrl:",finalUrl
	


	localSeenUrls = list(seenUrls)
	...
	if enableMetaTagSearch:
		...
		if debug: print "Feeds found by meta:",len(feedLinkTags),finalUrl
		...
		for feedLinkTag in feedLinkTags:
			...
			if debug: print "feedUrl:", fixedUrl, feedLinkTag.get("title","Sem Nome").encode('utf-8'),"link "+feedLinkTag["type"]
	...
	for feedAnchorTag in feedAnchorTags:
		...
		if feedParserSuccess and hasattr(feedFile, 'version') and feedFile.version != '':
			...
			if debug: print "...No!"
			if fixedUrl not in localSeenUrls:
				if isSameRootDomain(url, fixedUrl):
					if debug: print "Searching for more in url:", fixedUrl
					otherFeedUrls = getFeeds(fixedUrl, enableMetaTagSearch = False, seenUrls = localSeenUrls, deepLevel = deepLevel, debug = debug)
					localSeenUrls.append(fixedUrl)
					feedUrls  = feedUrls + otherFeedUrls
...
def getCandidateTags(url, debug=False) :
	...
	for openGraphTag in openGraphTags:
		candidateTags.append({"url" : openGraphTag['content'], "kind" : "openGraph"})
